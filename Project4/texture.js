...
var Blending = false;  // false = replacement; true = modulation;
var models4sort = [];  // for sort the sets
...
function loadModels() {
    ...
    try {
        if (inputTriangles == String.null)
            ...
            var uvToAdd; // vtx texture coords to add to the coord array
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                // set up the vertex and normal and texture arrays, define model center and axes
                inputTriangles[whichSet].glVertices = []; // flat coord list for webgl
                inputTriangles[whichSet].glNormals = []; // flat normal list for webgl
                inputTriangles[whichSet].glUVs = []; // flat texture coord list for webgl
                ...
                tmpobj.index = whichSet;
                tmpobj.center4depth = inputTriangles[whichSet].center;
                tmpobj.depth = 0;
                models4sort.push(tmpobj);
                //models4sort[whichSet].index = whichSet;
                //models4sort[whichSet].center4depth = inputTriangles[whichSet].center;
                ...
                // get texture images and init texture
                var imgPath = "https://ncsucgclass.github.io/prog4/" + inputTriangles[whichSet].material.texture;
                inputTriangles[whichSet].glTexture = initTexture(imgPath);
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                // locate texture attributes and uniforms
                vTexAttribLoc = gl.getAttribLocation(shaderProgram,"aTexturePosition");
                gl.enableVertexAttribArray(vTexAttribLoc);
                fSamplerULoc = gl.getUniformLocation(shaderProgram,"uSampler");
                ...
                fBlendingULoc = gl.getUniformLocation(shaderProgram, "uBlending");
                fAlphaULoc = gl.getUniformLocation(shaderProgram, "uAlpha");
...
function renderModels() {
    ...
    var e2c = vec3.create();
    var tmpLookat = vec3.create();
   for(var i = 0; i < models4sort.length; i++) {

        var currSet = models4sort[i].index;
        // update center4depth
        makeModelTransform(inputTriangles[currSet]);
        pvmMatrix = mat4.multiply(pvmMatrix,pvMatrix,mMatrix); // premultiply with pv matrix
        //vec3.transformMat4(models4sort[i].center4depth, models4sort[i].center4depth, pvmMatrix);
        var c = models4sort[i].center4depth;
        // console.log(c);
        var c4 = vec4.fromValues(c[0], c[1], c[2], 1.0);
        // console.log(c4);
        vec4.transformMat4(c4, c4, pvmMatrix);
        
        models4sort[i].center4depth = vec3.fromValues(c4[0], c4[1], c4[2]);


        // new lookat
        vec3.subtract(tmpLookat, Center, Eye);
        vec3.normalize(tmpLookat, tmpLookat);

        // eye to set center
        vec3.subtract(e2c, models4sort[i].center4depth, Eye);

        // depth
        models4sort[i].depth = vec3.dot(tmpLookat, e2c);
    }

    models4sort.sort(cmpbydepth);


    // render each triangle set
    var currSet; // the tri set and its material properties
    for(var i = 0; i < models4sort.length; i++) {
        whichTriSet = models4sort[i].index;
        ...
        // texture coords buffer: active and feed into vertex shader
        gl.bindBuffer(gl.ARRAY_BUFFER, textureBuffers[whichTriSet]);
        gl.vertexAttribPointer(vTexAttribLoc, 2, gl.FLOAT, false, 0, 0);
        
        // texture images: active and feed into fragent shader
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, inputTriangles[whichTriSet].glTexture);
        gl.uniform1i(fSamplerULoc, 0);
        gl.uniform1i(fBlendingULoc, Blending);
        gl.uniform1f(fAlphaULoc, inputTriangles[whichTriSet].material.alpha);

        // set z-buffer according to alpha
...
// compare funciton by depth
function cmpbydepth(a, b) {
